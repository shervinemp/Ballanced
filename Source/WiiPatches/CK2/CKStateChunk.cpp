...
#ifdef WII
    startPos = __builtin_bswap32(startPos);
#endif
    int currentPos = startPos;

    // Phase 1: Search from startPos to the end of the list
    if (currentPos != 0) {
        int curId = m_Data[currentPos];
#ifdef WII
        curId = __builtin_bswap32(curId);
#endif
        while (curId != identifier) {
            currentPos = m_Data[currentPos + 1];
#ifdef WII
            currentPos = __builtin_bswap32(currentPos);
#endif
            if (currentPos == 0)
                break;
            curId = m_Data[currentPos];
#ifdef WII
            curId = __builtin_bswap32(curId);
#endif
        }
...
#ifdef WII
        guid->d1 = __builtin_bswap32(guid->d1);
        guid->d2 = __builtin_bswap32(guid->d2);
#endif
    }
    m_ChunkParser->CurrentPos += 2;
    return i;
...
int CKStateChunk::ReadArray_LEndian(void **array) {
    ...
    if (parser && parser->CurrentPos < m_ChunkSize) {
        // Read array metadata
        int dataSizeBytes = m_Data[parser->CurrentPos++];
        int elementCount = m_Data[parser->CurrentPos++];
#ifdef WII
        dataSizeBytes = __builtin_bswap32(dataSizeBytes);
        elementCount = __builtin_bswap32(elementCount);
#endif
...
#ifdef WII
    classID = __builtin_bswap32(classID);
#endif
    ...
    if (m_ChunkVersion >= CHUNK_VERSION1) {
...
#ifdef WII
        sub->m_ChunkSize = __builtin_bswap32(sub->m_ChunkSize);
#endif
        CKBOOL hasFile = m_Data[m_ChunkParser->CurrentPos++];
#ifdef WII
        hasFile = __builtin_bswap32(hasFile);
#endif
        int idCount = m_Data[m_ChunkParser->CurrentPos++];
#ifdef WII
        idCount = __builtin_bswap32(idCount);
#endif
...
#ifdef WII
            managerCount = __builtin_bswap32(managerCount);
#endif
        }
        if (sub->m_ChunkSize != 0) {
        ...
        if (idCount > 0) {
            ...
            for(int i=0; i<idCount; i++) {
                sub->m_Ids->Data[i] = (int)__builtin_bswap32((uint32_t)sub->m_Ids->Data[i]);
            }
#endif
        }
        ...
        if (chunkCount > 0) {
            ...
            for(int i=0; i<chunkCount; i++) {
                sub->m_Chunks->Data[i] = (int)__builtin_bswap32((uint32_t)sub->m_Chunks->Data[i]);
            }
#endif
        }
        ...
        if (managerCount > 0) {
            ...
            for(int i=0; i<managerCount; i++) {
                sub->m_Managers->Data[i] = (int)__builtin_bswap32((uint32_t)sub->m_Managers->Data[i]);
            }
#endif
        }
...
int CKStateChunk::ReadInt() {
    ...
    int val = m_Data[m_ChunkParser->CurrentPos++];
#ifdef WII
    val = (int)__builtin_bswap32((uint32_t)val);
#endif
    return val;
}
...
float CKStateChunk::ReadFloat() {
    ...
    uint32_t val;
    memcpy(&val, &m_Data[m_ChunkParser->CurrentPos++], sizeof(uint32_t));
#ifdef WII
    val = __builtin_bswap32(val);
#endif
...
void CKStateChunk::ReadVector(VxVector &v) {
    ...
    uint32_t raw[3];
    memcpy(&raw, &m_Data[m_ChunkParser->CurrentPos], sizeof(VxVector));
#ifdef WII
    raw[0] = __builtin_bswap32(raw[0]);
    raw[1] = __builtin_bswap32(raw[1]);
...
void CKStateChunk::ReadMatrix(VxMatrix &mat) {
    ...
    uint32_t raw[16];
    memcpy(&raw, &m_Data[m_ChunkParser->CurrentPos], sizeof(VxMatrix));
#ifdef WII
    for(int i = 0; i < 16; i++) {
        raw[i] = __builtin_bswap32(raw[i]);
    }
#endif
...
#ifdef WII
        m_ChunkClassID = __builtin_bswap32(m_ChunkClassID);
#endif
        m_ChunkSize = buf[pos++];
#ifdef WII
        m_ChunkSize = __builtin_bswap32(m_ChunkSize);
#endif
        ++pos;
        int idCount = buf[pos++];
#ifdef WII
...
#ifdef WII
                for(int i = 0; i < idCount; i++) {
                    m_Ids->Data[i] = (int)__builtin_bswap32((uint32_t)m_Ids->Data[i]);
                }
#endif
                pos += idCount;
            }
...
#ifdef WII
                for(int i = 0; i < chunkCount; i++) {
                    m_Chunks->Data[i] = (int)__builtin_bswap32((uint32_t)m_Chunks->Data[i]);
                }
#endif
                pos += chunkCount;
            }
...
#ifdef WII
                for(int i = 0; i < managerCount; i++) {
                    m_Managers->Data[i] = (int)__builtin_bswap32((uint32_t)m_Managers->Data[i]);
                }
#endif
                pos += managerCount;
            }
...
int CKStateChunk::IterateAndDo(ChunkIterateFct fct, ChunkIteratorData *it) {
    ...
    while (chunkIndex < it->ChunkCount) {
        ...
        int entry = it->Chunks[chunkIndex];
#ifdef WII
        entry = __builtin_bswap32(entry);
#endif
        if (entry >= 0) {
            int *data = it->Data;
            int blockSize = data[entry];
#ifdef WII
            blockSize = __builtin_bswap32(blockSize);
#endif
            int entry2 = data[entry + 2];
#ifdef WII
            entry2 = __builtin_bswap32(entry2);
#endif
            if (blockSize > 0 && entry2 + 4 != blockSize) {
                child.ChunkVersion = static_cast<short>(entry2 >> 16);
                child.ChunkSize = data[entry + 3];
#ifdef WII
                child.ChunkSize = __builtin_bswap32(child.ChunkSize);
#endif
                child.IdCount = data[entry + 5];
#ifdef WII
                child.IdCount = __builtin_bswap32(child.IdCount);
#endif
                child.ChunkCount = data[entry + 6];
#ifdef WII
                child.ChunkCount = __builtin_bswap32(child.ChunkCount);
#endif
...
int CKStateChunk::ObjectRemapper(ChunkIteratorData *it) {
    ...
    if (it->ChunkVersion >= CHUNK_VERSION1) {
        ...
        for (int idx = 0; idx < it->IdCount;) {
...
#ifdef WII
                    CK_ID value = (CK_ID)__builtin_bswap32((uint32_t)it->Data[dataOffset]);
                    if (RemapObjectId(it, value)) {
                        it->Data[dataOffset] = (int)__builtin_bswap32((uint32_t)value);
                        ++remappedCount;
                    }
#else
            ...
            int count = it->Data[sequenceOffset];
#ifdef WII
            count = __builtin_bswap32(count);
#endif
...
#ifdef WII
                    CK_ID value = (CK_ID)__builtin_bswap32((uint32_t)it->Data[cursor]);
                    if (RemapObjectId(it, value)) {
                        it->Data[cursor] = (int)__builtin_bswap32((uint32_t)value);
                        ++remappedCount;
                    }
#else
...
#ifdef WII
    static constexpr CKDWORD OBJID_MARKER[3] = {0xC9C42BE3, 0xE3124213, 0xDCE9BAFC};
    static constexpr CKDWORD SEQ_MARKER[5] = {0x47BD92E1, 0x28662413, 0xCEB3EA13, 0xFCAE9178, 0x62459813};
#else
...
#ifdef WII
            CK_ID value = (CK_ID)__builtin_bswap32((uint32_t)it->Data[pos]);
            if (RemapObjectId(it, value)) {
                it->Data[pos] = (int)__builtin_bswap32((uint32_t)value);
                ++remappedCount;
            }
#else
...
#ifdef WII
                CKGUID guid(__builtin_bswap32(it->Data[offset]), __builtin_bswap32(it->Data[offset + 1]));
#else
                CKGUID guid(it->Data[offset], it->Data[offset + 1]);
#endif
                if (guid == it->Guid) {
#ifdef WII
                    int value = __builtin_bswap32(it->Data[offset + 2]);
                    if (value >= 0 && value < it->NbEntries) {
                        int mapped = it->ConversionTable[value];
                        if (mapped != value) {
                            it->Data[offset + 2] = __builtin_bswap32(mapped);
                            ++total;
                        }
                    }
#else
...
#ifdef WII
        CKGUID guid(__builtin_bswap32(it->Data[sequenceOffset + 1]), __builtin_bswap32(it->Data[sequenceOffset + 2]));
#else
...
#ifdef WII
                    int value = __builtin_bswap32(it->Data[cursor]);
                    if (value >= 0 && value < it->NbEntries) {
                        int mapped = it->ConversionTable[value];
                        if (mapped != value) {
                            it->Data[cursor] = __builtin_bswap32(mapped);
                            ++total;
                        }
                    }
#else
...
int CKStateChunk::ParameterRemapper(ChunkIteratorData *it) {
    ...
    while (currentPos + 1 < it->ChunkSize) {
        int curData = it->Data[currentPos];
#ifdef WII
        curData = __builtin_bswap32(curData);
#endif
        if (curData == 0x40) {
            const int headerStart = currentPos + 2;
            if (headerStart + 3 < it->ChunkSize) {
#ifdef WII
                CKGUID storedGuid(__builtin_bswap32(it->Data[headerStart]), __builtin_bswap32(it->Data[headerStart + 1]));
                int d2 = __builtin_bswap32(it->Data[headerStart + 2]);
#else
                CKGUID storedGuid(it->Data[headerStart], it->Data[headerStart + 1]);
                int d2 = it->Data[headerStart + 2];
#endif
                if (storedGuid == it->Guid && d2 == 1) {
#ifdef WII
                    int value = __builtin_bswap32(it->Data[headerStart + 3]);
                    if (value >= 0 && value < it->NbEntries)
                        it->Data[headerStart + 3] = __builtin_bswap32(it->ConversionTable[value]);
                    else
                        it->Data[headerStart + 3] = 0;
#else
...
#ifdef WII
        nextPos = __builtin_bswap32(nextPos);
#endif
